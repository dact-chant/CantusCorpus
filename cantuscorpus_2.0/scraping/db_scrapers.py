...
__changelog__ = {
    ...
    "0.0.3": {"updated_by": "Anna Dvorakova", "date": "2026-03-04", "changes": "add scraping of new values"},
}
...
class SourceData:
    '''
    POD class for chant sources. 
    Fields are picked based on their support among databases.
    '''
    def __init__(self):

        self._fields = [
            'title',            # in all dbs
            'siglum',           # in all dbs (sometimes called shelf-mark)
            'date',             # in MMMO is only date
            'century',          # in all dbs except MMMO
            'provenance',       # in all dbs
            'source_type',      # in all except CD
            'script',           # 
            'notation',         #
            ...
            'tradition',        # in PEM 
            'completeness',      # in PEM
            'n_cantus_chants',   # Is computed and filled later on
            'n_cantus_melodies', # Is computed and filled later on
            'images_link',       # MMMO, SEMM, 
            'description',      # CD, FCB, SEMM, CPL
        ]
    ...
    def to_csv_row(self, delimiter=','):
        ...
        for f in self._fields:
            ...
            elif fv.startswith('http'):  # Quote URLs
                output_fields.append('\"{}\"'.format(fv))
            else:
...
class _AbstractSourceScraper:
    ...
    def _url_to_http(self, source_url: str) -> str:
        """
        Converts a https:// URL to point to just http://.
        """
...
class CantusSKScraper(_AbstractSourceScraper):
    """
    Scraper for Slovak Early Music Database.
    """
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.DB_URL = 'https://cantus.sk'
        self._set_derived_drupal_db_urls()
        self._force_http = False # it has already https
    ...
    def source_data_from_source_soup(self, soup: BeautifulSoup) -> SourceData:
        ...
        #     'source_type'
        src_type_container = container.find('div', class_='field-name-field-source-type')
        if src_type_container:
            src_type = src_type_container.find('div', class_='field-item')
            if src_type: # in it is a <a href with the text of it being type
                src_type_a = src_type.find('a')
                if src_type_a:
                    source.source_type = src_type_a.text

        #     'script'
        script_container = container.find('div', class_='field-name-field-script')
        if script_container:
            script = script_container.find('div', class_='field-item')
            if script: # in it is a <a href with the text of it being type
                script_a = script.find('a')
                if script_a:
                    source.script = script_a.text
        
        #     'notation'
        notation_container = container.find('div', class_='field-name-field-notation')
        if notation_container:
            notation = notation_container.find('div', class_='field-item')
            if notation: # in it is a <a href with the text of it being type
                notation_a = notation.find('a')
                if notation_a:
                    source.notation = notation_a.text
        
        #     'description'
        description_container = container.find('div', class_='field-type-text-with-summary')
        if description_container:
            description = description_container.find('div', class_='field-item').text
            if description:
                # It has been in one or more <p> and <br> tags and we wanna get rid of them 
                # and also  have text without newlines.
                source.description = description.replace('\n', ' ').replace('\r', '').replace('<p>', ' ').replace('</p>', ' ').replace('<br>', ' ')
        
        #     'completeness'
        completeness_container = container.find('div', class_='field-name-field-complete')
        if completeness_container:
            completeness = completeness_container.find('div', class_='field-item')
            if completeness: 
                source.completeness = completeness.text
...
class IspanPLScraper(_AbstractSourceScraper):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.DB_URL = 'https://cantus.ispan.pl'
        self._set_derived_drupal_db_urls()
        self._force_http = False # it has already https
    ...
    def source_data_from_source_soup(self, soup: BeautifulSoup) -> SourceData:
        ...
        #     'images_link'
        images_link_container = container.find('div', class_='field-name-field-image-link')
        if images_link_container:
            images_link_a = images_link_container.find('a')
            if images_link_a:
                images_link = images_link_a['href']
                source.images_link = images_link
...
class MusmedDBScraper(_AbstractSourceScraper):
    ...
    def __init__(self, **kwargs):
        ...
        self._set_derived_drupal_db_urls()
        self._force_http = True # musmed is running on https but in the colleted chants it is still http
    ...
    def source_data_from_source_soup(self, soup: BeautifulSoup) -> SourceData:
        ...
        #     'order'
        order_container = container.find('div', class_='field-name-field-order')
        if order_container:
            order = order_container.find('div', class_='field-item')
            if order: # in it is a <a href with the text of it being type
                order_a = order.find('a')
                if order_a:
                    print('Found order: {}'.format(order_a))
                    source.order = order_a.text
        ...
        #     'description'
        description_container = container.find('div', class_='field-name-field-description')
        if description_container:
            description = description_container.find('div', class_='field-item').text
            if description:
                # It has been in one or more <p> and <br> tags and we wanna get rid of them 
        ...
        #     'images_link'
        images_link_container = container.find('div', class_='field-name-links-web1-web2')
        if images_link_container:
            images_link_a = images_link_container.find('a')
            if images_link_a:
                images_link = images_link_a['href']
...
class CantusDBScraper(_AbstractSourceScraper):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # We set these URLS
        self.DB_URL = 'https://cantusdatabase.org' # NOT 'https://cantus.uwaterloo.ca' any more
        self._set_derived_drupal_db_urls()
    ...
    def source_data_from_source_soup(self, soup: BeautifulSoup) -> SourceData:
        ...
        #     'description',
        mainpage_container = soup.find('dl')
        # Now we need to find <dt> with text "Description" 
        # and then get the following <dd class="markdown-field-display"> and its text.
        description_container = mainpage_container.find('dt', string='Description')
        if description_container:
            description_container = mainpage_container.find('dd', class_='markdown-field-display')
            if description_container:
                source.description = description_container.text.replace('\n', ' ').replace('\r', '').replace('<p>', ' ').replace('</p>', ' ').replace('<br>', ' ')

        #     'completeness'
        completness_container = mainpage_container.find('dt', string='Complete Source/Fragment')
        # Get text of closest dd with no class or other identificator
        if completness_container:
            completness_container = completness_container.find_next_sibling('dd')
            if completness_container:
                source.completeness = completness_container.text.strip()
...
class PemDBScraper(_AbstractSourceScraper):
    ...
    def source_data_from_source_soup(self, soup: BeautifulSoup) -> SourceData:
        ...
        src_type_container = container.find('div',class_='field--name-field-source-type')
        if src_type_container:
            src_type = src_type_container.find('div', class_='field__item')
            if src_type: # in it is a <a href with the text of it being type
                src_type_a = src_type.find('a')
                if src_type_a:
                    source.source_type = src_type_a.text
        ...
        #     'script'
        script_container = container.find('div', class_='field--name-field-type-script')
        if script_container:
            script = script_container.find('div', class_='field__item')
            if script: # in it is a <a href with the text of it being type
                script_a = script.find('a')
                if script_a:
                    source.script = script_a.text
        ...
        #     'notation'
        notation_container = container.find('div', class_='field--name-field-type-notation')
        if notation_container:
            notation = notation_container.find('div', class_='field__item')
            if notation: # in it is a <a href with the text of it being type
                notation_a = notation.find('a')
                if notation_a:
                    source.notation = notation_a.text
        ...
        #     'tradition'
        tradition_container = container.find('div', class_='field--name-field-tradition')
        if tradition_container:
            tradition = tradition_container.find('div', class_='field__item')
            if tradition: # in it is a <a href with the text of it being type
                tradition_a = tradition.find('a')
                if tradition_a:
                    source.tradition = tradition_a.text

        #     'description'
        description_container = container.find('div', class_='field--name-field-description')
        if description_container:
            description = description_container.find('div', class_='field__item').text
            if description:
                # It has been in one or more <p> and <br> tags and we wanna get rid of them 
                # and also  have text without newlines.
                source.description = description.replace('\n', ' ').replace('\r', '').replace('<p>', ' ').replace('</p>', ' ').replace('<br>', ' ')
        # Description is also in section Remarks - add that
        remarks_container = container.find('div', class_='field--name-field-remarks')
        if remarks_container:
            remarks = remarks_container.find('div', class_='field__item').text
            if remarks:
                # It has been in one or more <p> and <br> tags and we wanna get rid of them 
                # and also  have text without newlines.
                source.description = (source.description or '') + ' ' + remarks.replace('\n', ' ').replace('\r', '').replace('<p>', ' ').replace('</p>', ' ').replace('<br>', ' ')
        
        #     'images_link'
        # in <a> with text Image Gallery
        images_a = soup.find('a', string='Image Gallery')
        if images_a:
            images_link = images_a['href']
            source.images_link = self.DB_URL + images_link
        
        #     'completeness'
        completeness_container = container.find('div', class_='field--name-field-completeness')
        if completeness_container:
            completeness = completeness_container.find('div', class_='field__item')
            if completeness: # in it is a <a href with the text of it being type
                completeness_a = completeness.find('a')
                if completeness_a:
                    source.completeness = completeness_a.text
    ...
    def source_data_from_source_soup(self, soup: BeautifulSoup) -> SourceData:
        ...
        if description_container:
            ...
            if description:
                ...
                # and also  have text without newlines.
                source.description = description.replace('\n', ' ').replace('\r', '').replace('<p>', ' ').replace('</p>', ' ').replace('<br>', ' ')
        # Description can also be in the field "Notes" - add that
        notes_container = container.find('div', class_='field-name-field-notes')
        if notes_container:
            notes = notes_container.find('div', class_='field-item').text
            if notes:
                # It has been in one or more <p> and <br> tags and we wanna get rid of them 
                # and also  have text without newlines.
                source.description = (source.description or '') + ' ' + notes.replace('\n', ' ').replace('\r', '').replace('<p>', ' ').replace('</p>', ' ').replace('<br>', ' ')

        #     'notation'
        notation_container = container.find('div', class_='field-name-field-notation')
        if notation_container:
            # in it is a <a href with the text of it being type
            notation_a = notation_container.find('div', class_='field-item').find('a')
            if notation_a:
                source.notation = notation_a.text
        ...
        if src_type_container:
            # in it is a <a href with the text of it being type
            src_type_a = src_type_container.find('div', class_='field-item').find('a')
            if src_type_a:
                source.source_type = src_type_a.text
        ...
        if script_container:
            # in it is a <a href with the text of it being type
            script_a = script_container.find('div', class_='field-item').find('a')
            if script_a:
                source.script = script_a.text
...
class HungarianChantScraper(_AbstractSourceScraper):
    ...
    def source_data_from_source_soup(self, soup: BeautifulSoup) -> SourceData:
        ...
        tradition_container = container.find('div', class_='field-name-field-tradition')
        if tradition_container:
            tradition = tradition_container.find('div', class_='field-item')
            if tradition: # in it is a <a href with the text of it being type
                tradition_a = tradition.find('a')
                if tradition_a:
                    source.tradition = tradition_a.text
    ...
    def source_data_from_source_soup(self, soup: BeautifulSoup) -> SourceData:
        ...
        if description_container:
            ...
            if description:
                # It has been in one or more <p> and <br> tags and we wanna get rid of them 
                # and also  have text without newlines.
                source.description = description.replace('\n', ' ').replace('\r', '').replace('<p>', ' ').replace('</p>', ' ').replace('<br>', ' ').replace('<em>', ' ').replace('</em>', ' ')

        #     'images_link'
        images_link_container = container.find('div', class_='field-name-field-imageslink')
        if images_link_container:
            images_link_a = images_link_container.find('a')
            if images_link_a:
                images_link = images_link_a['href']
        ...
        #     'rite'
        rite_container = container.find('div', class_='field-name-field-rite')
        if rite_container:
            # in it is a <a href with the text of it being type
            rite_a = rite_container.find('div', class_='field-item').find('a')
            if rite_a:
                source.rite = rite_a.text
...
class AustriaManusScraper(_AbstractSourceScraper):
    ...
    def source_data_from_source_soup(self, soup: BeautifulSoup) -> SourceData:
        ...
        #     'images_link'
        images_link_container = container.find('div', class_='field-name-field-link')
        if images_link_container:
            images_link_a = images_link_container.find('a')
            if images_link_a:
                images_link = images_link_a['href']
        ...
        #     'description'
        description_container = container.find('div', class_='field-name-field-notes')
        if description_container:
            description = description_container.find('div', class_='field-item').text
            if description:
                # It has been in one or more <p> and <br> tags and we wanna get rid of them 
        ...
        #     'cursus' - austriamanus does not annotate cursus
        return source
...
class HymnologicaScraper(_AbstractSourceScraper):
    ...
    def __init__(self, **kwargs):
        ...
        #self._set_derived_drupal_db_urls()
        self._force_http = False # HYM already on https
    ...
    def source_data_from_source_soup(self, soup: BeautifulSoup) -> SourceData:
        ...
        # 'source_type'
        src_type_container = container_section.find('div', class_='field-name-field-source-type')
        if src_type_container:
            src_type = src_type_container.find('div', class_='field-item')
            if src_type: # in it is a <a href with the text of it being type
                src_type_a = src_type.find('a')
        ...
        # 'notation'
        notation_container = container_section.find('div', class_='field-name-field-notation')
        if notation_container:
            notation_container = notation_container.find('div', class_='field-item')
            if notation_container:
                source.notation = notation_container.text
        
        # 'images_link'
        images_link_container = container_section.find('div', class_='field-name-field-image-link')
        if images_link_container:
            images_link_a = images_link_container.find('a')
            if images_link_a:
                images_link = images_link_a['href']
        ...
        # 'description'
        description_container = container_section.find('div', class_='field-name-field-notes')
        if description_container:
            description = description_container.find('div', class_='field-item').text
            if description:
                # It has been in one or more <p> and <br> tags and we wanna get rid of them 
